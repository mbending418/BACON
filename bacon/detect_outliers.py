...
from linear_model import LinearModelPrediction
...
class BACON:
    ...
    @cached_property
    def linear_model(self) -> LinearModelPrediction | None:
        if self.y_data is None:
            return None
        return LinearModelPrediction(x_data=self.x_data, y_data=self.y_data)

    def model_subset(self, indexes: list[int]) -> LinearModelPrediction:
        """
        ...
        return LinearModelPrediction(x_in, y_in)
    ...
    def initial_multivariate_subset(
        self,
        m: int | None = None,
        selection_method: Literal["mahalanobis", "median"] = "mahalanobis",
    ) -> list[int]:
        """
        return the initial multivariate subset for the BACON algorithm
        specifically return the m indexes with the lowest "distance"
    ...
    def remove_multivariate_outliers(
        ...
        """
        current_subset = self.initial_multivariate_subset(
    ...
    def initial_regression_subset(
        ...
        """
        return the initial subset for the BACON algorithm for Linear Regression
    ...
    def remove_regression_outliers(
        ...
        if alpha_t is None:
            alpha_t = alpha_chi
        current_subset = self.initial_regression_subset(
            m=m,
